...
import { Loader2, UploadCloud, MapPin, CheckCircle, Info } from 'lucide-react';
import dynamic from 'next/dynamic';
import { editListingSeller } from '@/app/dashboard/actions';
...
export default function EditListingForm({ listing }: { listing: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await editListingSeller(formData);
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-200 shadow-xl shadow-slate-200/50 space-y-8 relative overflow-hidden">
      ...
      <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl flex items-start gap-3">
        ...
        <div className="text-sm text-amber-800">
          <strong>Perhatian:</strong> Menyimpan perubahan pada lahan ini akan mengubah statusnya kembali menjadi <span className="font-bold">Antrean Review</span>. Admin perlu meninjau kembali perubahan Anda sebelum lahan tayang di halaman publik.
        </div>
      </div>
      ...
      {success && (
        <div className="bg-emerald-50 text-emerald-700 p-4 rounded-xl text-sm font-bold border border-emerald-100 flex items-center">
          <CheckCircle className="w-5 h-5 mr-2" /> Data berhasil diperbarui! Mengalihkan ke dashboard...
        </div>
      )}
      ...
      <div className="space-y-6 pt-6 border-t border-slate-100">
        ...
        {currentPhoto && (
          <div className="mb-4">
            ...
            <div className="relative w-full h-48 md:w-64 rounded-xl overflow-hidden border border-slate-200 shadow-sm">
        ...
        <div>
          ...
          />
          <p className="text-xs text-slate-500 mt-2 font-medium">Biarkan kosong jika Anda tidak ingin mengubah foto lahan.</p>
        </div>
      </div>
      ...
      <div className="flex justify-end pt-8 border-t border-slate-100">
        ...
        >
          {loading ? (
            ...
            <><UploadCloud className="w-5 h-5 mr-2" /> Simpan & Ajukan Review</>
          )}
        </button>
      </div>
